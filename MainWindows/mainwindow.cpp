    ...
    ui(new Ui::MainWindow)
{
    ...
    connect(VideoHandle::getInstance(), SIGNAL(sendVideoFPS(QPixmap)), this, SLOT(showVideoFPS(QPixmap)));
}
...
void MainWindow::on_OpenImageButton_clicked()
{
    ...
    if(PicNum != 1) return;
    else if(PicNum == 1) ui->ImageAddressLineEdit->setText(filePaths.first());

    VideoHandle::getInstance()->addVideoPath(filePaths.first());

}

void MainWindow::showVideoFPS(QPixmap imageFPS)
{
    ui->ImageLabel->setPixmap(imageFPS);
}
